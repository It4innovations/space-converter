...
unsigned long long bytesToUInt64_bigEndian(unsigned char* b) {
...
long long bytesToInt64_bigEndian(unsigned char* b) {
	long long temp = 0;
	long long res = 0;
 ...
 * */
long long bytesToLong_bigEndian(unsigned char* b) {
	long long temp = 0;
	long long res = 0;
...
void longToBytes_bigEndian(unsigned char *b, unsigned long long num)
{
	b[0] = (unsigned char)(num>>56);
	b[1] = (unsigned char)(num>>48);
	b[2] = (unsigned char)(num>>40);
...
long long doubleToOSEndianLong(double value)
{
	ldouble buf;
	buf.value = value;
	return buf.lvalue;
...
{
	//long long lvalue = doubleToBigEndianLong(value);
...
unsigned char numberOfLeadingZeros_Long(long long i) {
	 if (i == 0)
		return 64;
	unsigned char n = 1;
	int x = (int)(((unsigned long long)i) >> 32);
	if (x == 0) { n += 32; x = (int)i; }
...
unsigned char getLeadingNumbers_Long(long long v1, long long v2)
{
	long long v = v1 ^ v2;
...
long long bytesToLong(unsigned char* bytes)
{
	ldouble buf;
