...
int computeByteSizePerIntValue(long long valueRangeSize)
{
...
long long computeRangeSize_int(void* oriData, int dataType, size_t size, int64_t* valueRangeSize)
{
	size_t i = 0;
	...
	{
		unsigned long long* data = (unsigned long long*)oriData;
	...
	{
		long long* data = (long long *)oriData;
		long long data_; 
		min = data[0], max = min;
		computeMinMax(data);
	}
...
double getRealPrecision_int(long long valueRangeSize, int errBoundMode, double absErrBound, double relBoundRatio, int *status)
{
	int state = SZ_SCES;
	double precision = 0;
	if(errBoundMode==ABS||errBoundMode==ABS_OR_PW_REL||errBoundMode==ABS_AND_PW_REL)
