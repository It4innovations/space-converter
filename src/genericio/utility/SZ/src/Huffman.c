...
{			
	...
	huffmanTree->code = (unsigned long long**)malloc(huffmanTree->stateNum*sizeof(unsigned long long*));
    ...
    memset(huffmanTree->code, 0, huffmanTree->stateNum*sizeof(unsigned long long*));
 ...
 * */
void build_code(HuffmanTree *huffmanTree, node n, int len, unsigned long long out1, unsigned long long out2)
{
	if (n->t) {
		huffmanTree->code[n->c] = (unsigned long long*)malloc(2*sizeof(unsigned long long));
		if(len<=64)
		{
...
void encode(HuffmanTree *huffmanTree, int *s, size_t length, unsigned char *out, size_t *outSize)
{
	...
	unsigned char *p = out;
	int lackBits = 0;
	//long long totalBitSize = 0, maxBitSize = 0, bitSize21 = 0, bitSize32 = 0;
	for (i = 0;i<length;i++) 
	{
		//state = 0;
		//state = (state | s[i])<<8;
		...
		else
		{
			*p = (*p) | (unsigned char)((huffmanTree->code[state])[0] >> (64 - lackBits));			
			if(lackBits < bitSize)
			{
				p++;
				//(*outSize)++;
				long long newCode = (huffmanTree->code[state])[0] << lackBits;
