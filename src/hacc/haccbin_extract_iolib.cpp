...
//   get_particle_mass(id) -> per-particle mass field of the HACC binary record
//                            (HACC code units), single particle type
//   get_particle_rho(id)  -> per-particle rho field of the HACC binary record
//   get_particle_hsml(id) -> per-particle hh (smoothing length) field of the record
//   particle id space     -> rank-local, single type: ids index this rank's contiguous
//                            slice [g_start_particles, g_start_particles + g_num_particles)
...
#pragma pack(pop)  // Restore default packing
		...
		void init_lib(std::string basefile, int world_rank, int world_size) {
			...
			size_t particles_per_process = g_total_particles / world_size;
			g_start_particles = (size_t)world_rank * particles_per_process;
