...
{	
	...
	unsigned long long radiusIndex;
	...
	for(i=2;i<dataLength;i++)
	{
		...
		if(i%confparams_cpr->sampleDistance==0)
		{
			...
			radiusIndex = (unsigned long long)((pred_err/realPrecision+1)/2);
...
void SZ_compress_args_float_NoCkRngeNoGzip_1D_pwr_pre_log(unsigned char** newByteData, float *oriData, double pwrErrRatio, size_t dataLength, size_t *outSize, float min, float max){
    ...
    if(!positive){
		...
		// compress signs
		unsigned long long signSize = sz_lossless_compress(confparams_cpr->losslessCompressor, confparams_cpr->gzipMode, signs, dataLength, &comp_signs);
