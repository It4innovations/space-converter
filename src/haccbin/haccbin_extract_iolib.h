...
#include <vector>
 ...
 // Namespace haccbin provides functionalities for handling HACCBin data.
namespace haccbin {

    // Enum defining particle types in the Tipsy format.
    enum HACCBinParticleType {
        DarkMatter = 0,
        Baryon,
        BaryonStart,
        BaryonWind,
    ...
    enum HACCBinBlockType {
        Pos = 0,        // Positions (x, y, z) are in units of Mpc/h
        Vel,            // Velocities (vx, vy, vz) are in units of km/s
        Mass,           // Mass is in internal code units        
        UU,             // Internal energy (uu) is in units of (km/s)^2
        HH,             // SPH smoothing length (hh) is in units of Mpc/h
        MU,             // Molecular weight (mu) is dimensionless
        Rho,            // Density (rho) is in units of h^2*Msolar/Mpc^3 where Msolar is the mass of the Sun
        Phi,            // Gravitational potential (phi) is in internal code units
        Id,
        Mask,
    ...
    // Namespace io contains I/O operations and utilities for HACCBin data processing.
    namespace io {

        // Print the steps executed on the CPU during the HACCBin process.
        ...
        // Initialize the HACCBin library.
        void init_lib(std::string basefile, int world_rank, int world_size);

        // Finalize and clean up the HACCBin library.
        void finish_lib();

        // Set parameters for the HACCBin.
        ...
        //void haccbin_set_parameter(int ICFormat, int SnapFormat, int NumFilesWrittenInParallel, int MaxMemSize, double BufferSize, double PartAllocFactor, int TotBHs);

        // Read a parameter file for HACCBin.
        ...
        //void haccbin_read_parameter_file(char* fname, char* tag[], void** addr, int* id, int nt);

        // Read initial condition data from a file.
        // @param fname: Name of the file containing initial conditions.
        //void haccbin_read_ic(std::string fname);

        // Initialize memory allocation for HACCBin.
        //void haccbin_mymalloc_init();

        // Set units for HACCBin data.
        //void haccbin_set_units();
