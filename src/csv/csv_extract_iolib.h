...
#include <vector>
 ...
 // Namespace csv provides functionalities for handling CSV data.
namespace csv {

    // Enum defining particle types in the Tipsy format.
    enum CSVParticleType {
        Gas = 0,        // Gas particles
    ...
    // Namespace io contains I/O operations and utilities for CSV data processing.
    namespace io {

        // Print the steps executed on the CPU during the CSV process.
        ...
        // Finalize and clean up the CSV library.
        void finish_lib();

        // Set parameters for the CSV.
        ...
        //void csv_set_parameter(int ICFormat, int SnapFormat, int NumFilesWrittenInParallel, int MaxMemSize, double BufferSize, double PartAllocFactor, int TotBHs);

        // Read a parameter file for CSV.
        ...
        //void csv_read_parameter_file(char* fname, char* tag[], void** addr, int* id, int nt);

        // Read initial condition data from a file.
        // @param fname: Name of the file containing initial conditions.
        //void csv_read_ic(std::string fname);

        // Initialize memory allocation for CSV.
        //void csv_mymalloc_init();

        // Set units for CSV data.
        //void csv_set_units();
