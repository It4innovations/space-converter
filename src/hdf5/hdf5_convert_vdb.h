...
#include "convert_vdb.h"
 ...
 // Namespace hdf5 contains functionality for handling VDB data conversion specific to the HDF5 format (mainly for testing).
namespace hdf5 {

    // ConvertVDBHDF5 is a derived class from common::vdb::ConvertVDBBase.
    // This class implements methods for converting particle data from HDF5/Excel format into VDB format.
    class ConvertVDBHDF5 : public common::vdb::ConvertVDBBase {
    public:
        ...
        float get_particle_norm_value(int blocknr, uint64_t id) override;

        int get_particle_value(int blocknr, uint64_t id, float* out_value) override;
        int get_particle_value_comp(int blocknr, uint64_t id) override;
        ...
        double get_particle_rho(uint64_t id) override;
        ...
        void get_types_and_blocks(std::vector<int>& types_and_blocks) override;
        ...
        std::string get_particle_data_types(std::vector<int>& types_and_blocks) override;

    }; // class ConvertVDBHDF5
