...
//////////////////////////////////////////////////////////////////////////////////////
// Helper to determine the system's endianness
bool isSystemLittleEndian() {
	uint16_t test = 0x1;
	return *(reinterpret_cast<uint8_t*>(&test)) == 0x1;
}

// Swap bytes for a 32-bit integer
...
float swapEndianFloat(float value) {
	uint32_t intValue;
	std::memcpy(&intValue, &value, sizeof(float)); // Copy float bits into a 32-bit integer
	intValue = swapEndian32(intValue);            // Swap the byte order of the integer
	std::memcpy(&value, &intValue, sizeof(float)); // Copy back to float
	return value;
}

void replaceCharacter(std::string& str, char target, char replacement) {
...
bool PartialTipsyFile::loadPartialAUX(const std::string& full_path, const unsigned int beginParticle,
	...
	if (!file_path.has_parent_path()) {
		std::cerr << "Invalid path provided: " << full_path << std::endl;
		return false;
	}

	std::string directory = file_path.parent_path().string();
	std::string base_name = file_path.filename().string();
	...
	for (const auto& entry : fs::directory_iterator(directory)) {
		...
		if (fs::is_regular_file(entry)) {
			...
			// Check if the file starts with the base name followed by a dot
			std::string base_name2 = base_name + ".";
			if (file_name.compare(0, base_name2.size(), base_name2) == 0)	{
				// Extract the part after the base name and the dot
				std::string suffix = file_name.substr(base_name.size() + 1);
	...
	for (int i = 0; i < aux_values.size(); i++) {
		...
		std::string filename_aux = directory + "/" + base_name + "." + aux_values[i].name;

		// Open the binary file for reading
		std::ifstream file(filename_aux, std::ios::binary | std::ios::ate);
		if (!file) {
			std::cerr << "Error: Could not open the file " << filename_aux << std::endl;
			continue;
		}
		...
		if (h.ndark > 0)
			aux_values[i].darks.resize(h.ndark * aux_values[i].num_components);

		if (h.nstar > 0)
			aux_values[i].stars.resize(h.nstar * aux_values[i].num_components);

		for (int c = 0; c < aux_values[i].num_components - num_components + 1; c++) {
			if (c == 1) {
				replaceCharacter(aux_values[i].name, 'x', 'y');
				filename_aux = directory + "/" + base_name + "." + aux_values[i].name;
			}
			else if (c == 2) {
				replaceCharacter(aux_values[i].name, 'y', 'z');
				filename_aux = directory + "/" + base_name + "." + aux_values[i].name;
			}

			file.open(filename_aux, std::ios::binary);
			if (!file) {
				std::cerr << "Error: Could not reopen the file " << filename_aux << std::endl;
				continue;
			}

			// Move the file pointer to skip the first 'n + 1'		
			file.seekg(beginParticle * sizeof(float) * num_components + sizeof(int), std::ios::beg);
			if (!file) {
				std::cerr << "Error: Failed to seek in the file" << filename_aux << std::endl;
				continue;
			}

			float v = 0;
			// Read the remaining floats into a vector
...
#ifdef _WIN32
	h.nsph = max(min(fullHeader.nsph, endParticle) - (int64_t)beginParticle, (int64_t)0);
	h.ndark = max(min(endParticle, fullHeader.nsph + fullHeader.ndark) - (int64_t)max(fullHeader.nsph, beginParticle), (int64_t)0);
	h.nstar = max(endParticle - (int64_t)max(fullHeader.nsph + fullHeader.ndark, beginParticle), (int64_t)0);
#else
